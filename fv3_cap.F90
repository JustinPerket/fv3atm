...
!-----------------------------------------------------------------------------
    ...
    ! JP add, now three run phases
    call ESMF_LogWrite('Model Advance phase2: after fcstComp run phase2', ESMF_LOGMSG_INFO, rc=rc) ! TMP
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
    ...
    call ESMF_LogWrite('Model Advance phase3: after fcstcomp run ', ESMF_LOGMSG_INFO, rc=rc) ! TMP
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
    ! JP end
